...
const Details = () => {
  ...
  const fetchDetails = async () => {
    try {
      const res = await fetch(`${prefix}/getrecord?detailsId=${detailsId}`, {
        method: "GET",
        headers: {
  ...
  const analyzeDetails = () => {
    ...
    const totalWorkoutTime = Math.ceil(
      details.reduce((acc, detail) => acc + detail.duration, 0) / 60
    );
    // console.log(details.endTime, details.startTime);
    const totalTime = Math.ceil(
      (new Date(record.endTime) - new Date(record.startTime)) / 1000 / 60
    );
  ...
  const handleSave = async (_id) => {
    try {
      const res = await fetch(`${prefix}/editDetails`, {
        method: "PUT",
                  ...
                  <>
                    ...
                    <td className="px-4 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm({ ...editForm, exercise: e.target.value })
                        }
                      />
                    </td>
                    <td className="px-4 py-2">
        ...
        <div className="bg-white shadow-md rounded-lg p-6 border border-gray-200 w-full max-w-md mx-auto my-6">
          ...
          <h2 className="text-lg font-semibold text-gray-700 mb-2">
            ...
            <span className="font-bold text-red-500">
              {summary.totalWeight} lb
            </span>
          </h2>
          <h2 className="text-lg font-semibold text-gray-700 mb-2">
            💪 Active Workout Time:{" "}
            <span className="font-bold text-green-600">
              {summary.totalWorkoutTime} mins
            </span>
          </h2>
          <h2 className="text-lg font-semibold text-gray-700">
            📊 Training Efficiency:{" "}
            <span className="font-bold text-purple-600">
              {summary.trainingPercentage.toFixed(2)}%
            </span>
          </h2>
        </div>
