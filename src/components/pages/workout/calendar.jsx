...
const Calendar = () => {
  ...
  const getRecords = async () => {
    try {
      ...
      const res = await fetch(`${prefix}?userId=${userId}&date=${date}`);
      ...
      for (let record of data) {
        ...
        const cell = cellsOfMonth.find(
          (cell) => cell.day === day && currentMonth === month
        );
        if (cell) {
    ...
    try {
      const res = await fetch(`${prefix}/delete`, {
        ...
        body: JSON.stringify({
          ...
          detailsId: selectedRecord.details || "",
        }),
      });
      if (res.ok) {
  ...
  const startRecord = async () => {
    try {
      ...
      const res = await fetch(`${prefix}/initrecord`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div>
      <div className="flex flex-col p-1 m-5 max-w-4xl mx-auto bg-white rounded-lg shadow-xl">
        ...
        <div className="flex flex-row justify-center">
          <button
            className="m-3 px-3 py-1 rounded-xl bg-blue-300 hover:bg-blue-500"
            onClick={prevMonth}
          >
            Prev
          </button>
          ...
          >
            ...
            <div>
              ...
              <input
                type="time"
                id="time"
                value={new Date(selectedRecord.date).toLocaleTimeString(
                  "en-GB",
                  {
                    hour: "2-digit",
                    minute: "2-digit",
