...
const Recording = () => {
  ...
  useEffect(() => {
    ...
    const selected = workouts.find(
      (workout) => workout.portion === selectedWorkout
    );
    if (selected) {
  ...
  const handleRecording = async () => {
    ...
    // resume the recording
      ...
      if (
        selectedWorkout === "SELECT PORTION" ||
      ...
      <div className="w-full max-w-md p-6 bg-white rounded-2xl shadow-lg space-y-6">
        ...
        <h1 className="text-2xl font-bold text-center">
          {isRunning ? (
            <>
              Recording:{" "}
              <span className="text-sky-500">{timeDisplay(duration)}</span>
            </>
          ) : (
            <>
              Cooling Down:{" "}
              <span className="text-red-500">{timeDisplay(coolingTime)}</span>
            </>
          )}
        </h1>
          ...
          <div>
            ...
            >
              {workouts.map((workout, index) => (
                <option value={workout.portion} key={index}>
                  {workout.portion}
                </option>
              ))}
            </select>
            ...
            >
              {exercises &&
                exercises.map((exercise, index) => (
                  <option value={exercise.name} key={index}>
                    {exercise.name}
                  </option>
                ))}
            </select>
          ...
          {finishedSets.length === 0 ? (
            ...
            <ul className="space-y-2 max-h-48 overflow-y-auto pr-1">
              {finishedSets.map((set, index) => (
                <li
                  key={index}
                  className="text-sm bg-teal-100 p-2 rounded-md shadow-sm"
                >
                  <p>
                    <span className="font-medium">Portion:</span> {set.portion}
                  </p>
                  <p>
                  ...
                  <p>
                    <span className="font-medium">Weight:</span> {set.weight}{" "}
                    lbs
                  </p>
                  <p>
                    <span className="font-medium">Reps:</span> {set.repetition}
                  </p>
                </li>
              ))}
            </ul>
